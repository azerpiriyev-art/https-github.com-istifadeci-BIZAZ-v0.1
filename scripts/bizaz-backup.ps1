...
$oneDriveBackupDir = "C:\Users\USER\OneDrive\BIZAZ-Backups"
...
$oneDriveBackup = Join-Path $oneDriveBackupDir $backupName
$oneDriveChecksum = Join-Path $oneDriveBackupDir $checksumName
...
Write-Host "8. Copy verified backup to OneDrive"

Copy-Item $localBackup $oneDriveBackup -Force
Copy-Item $localChecksum $oneDriveChecksum -Force

Write-Host "9. Verify OneDrive copy"

$oneDriveHash = Get-FileHash $oneDriveBackup -Algorithm SHA256

if ($oneDriveHash.Hash -ne $hash.Hash) {
    throw "OneDrive backup SHA256 mismatch."
}

Write-Host "ONEDRIVE_SHA256_MATCH: True"

Write-Host "10. Retention - remove backups older than 30 days"
...
Get-ChildItem "$oneDriveBackupDir\*.dump" -File |
    Where-Object { $_.LastWriteTime -lt $cutoff } |
    Remove-Item -Force

Get-ChildItem "$oneDriveBackupDir\*.sha256" -File |
    Where-Object { $_.LastWriteTime -lt $cutoff } |
    Remove-Item -Force

Write-Host "11. Cleanup complete"

Write-Host "BACKUP_FILE:" $localBackup
Write-Host "BACKUP_SIZE:" (Get-Item $localBackup).Length
Write-Host "BACKUP_SHA256:" $hash.Hash
Write-Host "ONEDRIVE_FILE:" $oneDriveBackup
